...
@property (nonatomic, strong) NSString *socketDirectory; // Cached socket directory
@property (nonatomic, strong) NSXPCListener *xpcListener;  // XPC listener for extension communication
...
- (void)setupControlSocket {
    ...
    // This directory is always accessible, writable, and short enough for socket paths
    self.socketDirectory = NSTemporaryDirectory();
    NSLog(@"[HIAHKernel] Using NSTemporaryDirectory for sockets: %@", self.socketDirectory);
    
    // Short socket name
    NSString *socketName = @"k.s";
    self.controlSocketPath = [self.socketDirectory stringByAppendingPathComponent:socketName];
    ...
    if (strlen(fullSocketPath) >= sizeof(addr.sun_path)) {
        NSLog(@"[HIAHKernel] Control socket path too long: %@", self.controlSocketPath);
        close(serverSock);
        return;
    }
    ...
    if (bind(serverSock, (struct sockaddr *)&addr, sizeof(addr)) == 0) {
        ...
        NSLog(@"[HIAHKernel] Failed to bind control socket at %@: %s", self.controlSocketPath, strerror(errno));
        close(serverSock);
    }
}
...
- (void)registerProcess:(HIAHProcess *)process {
    ...
    NSLog(@"[HIAHKernel] Registered process %d (%@)", process.pid, process.executablePath);
...
- (HIAHProcess *)processForRequestIdentifier:(NSUUID *)uuid {
    ...
    [self.processTable enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, HIAHProcess *obj, BOOL *stop) {
...
completion:(void (^)(pid_t pid, NSError *error))completion {
    ...
    // If the path points to a .app bundle, we need to find the executable inside it
    BOOL isDirectory = NO;
    if ([fm fileExistsAtPath:path isDirectory:&isDirectory] && isDirectory && [path hasSuffix:@".app"]) {
        ...
        if (executableName) {
            ...
            NSString *candidatePath = [path stringByAppendingPathComponent:executableName];
            if ([fm fileExistsAtPath:candidatePath]) {
                ...
                candidatePath = [[path stringByAppendingPathComponent:@"Contents/MacOS"] stringByAppendingPathComponent:executableName];
                if ([fm fileExistsAtPath:candidatePath]) {
                    actualExecutablePath = candidatePath;
                    NSLog(@"[HIAHKernel] Found executable at: %@", actualExecutablePath);
                } else {
                    NSLog(@"[HIAHKernel] ERROR: Could not find executable '%@' in bundle", executableName);
                    if (completion) {
                        NSError *error = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                                             code:HIAHKernelErrorInvalidPath 
                                                         userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Executable '%@' not found in bundle", executableName]}];
                        completion(-1, error);
                    }
                    return;
                }
            ...
            if (completion) {
                NSError *error = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                                     code:HIAHKernelErrorInvalidPath 
                                                 userInfo:@{NSLocalizedDescriptionKey: @"No CFBundleExecutable in Info.plist"}];
                completion(-1, error);
            }
            return;
        }
    ...
    if (![fm fileExistsAtPath:path]) {
        ...
        if (completion) {
            NSError *error = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                                 code:HIAHKernelErrorInvalidPath 
                                             userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Executable not found: %@", path]}];
            completion(-1, error);
        }
        return;
    }
    ...
    // CRITICAL: Ensure executable has correct permissions
        ...
        if (permError) {
            NSLog(@"[HIAHKernel] Warning: Could not set executable permissions: %@", permError);
        } else {
    ...
    // Short socket name
    NSString *socketName = [NSString stringWithFormat:@"%d.s", arc4random() % 100];
    ...
    if (serverSock < 0) {
        ...
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                           ...
                                           userInfo:@{NSLocalizedDescriptionKey: @"Failed to create output socket"}];
            completion(-1, err);
        }
        return;
    }
    ...
    // Use absolute path instead of chdir (iOS device doesn't allow chdir to app group)
    ...
    if (bind(serverSock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        NSLog(@"[HIAHKernel] Failed to bind stdout socket at %@: %s", socketPath, strerror(errno));
        close(serverSock);
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                                   code:HIAHKernelErrorSocketCreationFailed 
                                               userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Failed to bind socket: %s", strerror(errno)]}];
            completion(-1, err);
        }
        return;
    }
    ...
    if (!extensionClass) {
        ...
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                           ...
                                           userInfo:@{NSLocalizedDescriptionKey: @"NSExtension not available"}];
            completion(-1, err);
        }
        return;
    }
    ...
    NSString *appexPath = [plugInsPath stringByAppendingPathComponent:@"HIAHProcessRunner.appex"];
    NSBundle *extensionBundle = [NSBundle bundleWithPath:appexPath];
    
    if (!extensionBundle) {
        HIAHLogError(HIAHLogKernel, "Extension bundle not found at %s", [appexPath UTF8String]);
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                               code:HIAHKernelErrorExtensionNotFound 
                                           userInfo:@{NSLocalizedDescriptionKey: @"ProcessRunner extension not bundled"}];
            completion(-1, err);
        }
        return;
    }
    ...
    HIAHLogDebug(HIAHLogKernel, "Loading extension with identifier: %s", [extId UTF8String]);
    
    SEL selector = NSSelectorFromString(@"extensionWithIdentifier:error:");
    NSMethodSignature *signature = [extensionClass methodSignatureForSelector:selector];
    ...
    if (!extension) {
        HIAHLogError(HIAHLogKernel, "Failed to load extension %s: %s", [extId UTF8String], error ? [[error description] UTF8String] : "(null)");
        if (completion) completion(-1, error);
        return;
    }
    ...
    NSMutableDictionary *env = environment ? [environment mutableCopy] : [NSMutableDictionary dictionary];
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    [item performSelector:NSSelectorFromString(@"setUserInfo:") withObject:userInfo];
#pragma clang diagnostic pop
    
    void (^completionBlock)(NSUUID *) = ^(NSUUID *requestIdentifier) {
        if (!requestIdentifier) {
            HIAHLogError(HIAHLogKernel, "Failed to start extension request for %s", [path UTF8String]);
            if (completion) {
                NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
        ...
        HIAHProcess *vproc = [HIAHProcess processWithPath:path arguments:arguments environment:environment];
        vproc.requestIdentifier = requestIdentifier;
        
        // Try to get physical PID
        pid_t physicalPid = -1;
        if ([extension respondsToSelector:NSSelectorFromString(@"pidForRequestIdentifier:")]) {
            NSInvocation *pidInv = [NSInvocation invocationWithMethodSignature:
                                    [extension methodSignatureForSelector:NSSelectorFromString(@"pidForRequestIdentifier:")]];
        ...
        HIAHLogInfo(HIAHLogKernel, "Spawned guest process (Virtual PID: %d, Physical PID: %d)", 
                    vproc.pid, physicalPid);
        if (completion) completion(vproc.pid, nil);
    };
    
    // 4. Begin Request
    SEL beginRequestSelector = NSSelectorFromString(@"beginExtensionRequestWithInputItems:completion:");
    NSArray *inputItems = @[item];
    
    HIAHLogDebug(HIAHLogKernel, "Sending extension request for path: %s", [path UTF8String]);
    
    // Verify the extension responds to the selector
    if (![extension respondsToSelector:beginRequestSelector]) {
        HIAHLogError(HIAHLogKernel, "Extension does not respond to beginExtensionRequestWithInputItems:completion:");
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                               code:HIAHKernelErrorSpawnFailed 
                                           userInfo:@{NSLocalizedDescriptionKey: @"Extension does not support beginExtensionRequestWithInputItems:completion:"}];
            completion(-1, err);
        }
        return;
    }
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    @try {
        [extension performSelector:beginRequestSelector withObject:inputItems withObject:completionBlock];
        HIAHLogDebug(HIAHLogKernel, "Extension request invoked successfully");
        ...
        if (completion) {
            NSError *err = [NSError errorWithDomain:HIAHKernelErrorDomain 
                                               code:HIAHKernelErrorSpawnFailed 
                                           userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Extension exception: %@", exception.reason]}];
            completion(-1, err);
        }
    }
#pragma clang diagnostic pop
