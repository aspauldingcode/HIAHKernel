...
- (void)viewDidLoad {
  ...
  self.logView.font = [UIFont monospacedSystemFontOfSize:12
  ...
  [NSLayoutConstraint activateConstraints:@[
    ...
    [self.statusLabel.trailingAnchor
        constraintEqualToAnchor:self.view.trailingAnchor
                       constant:-20],
    [self.browseButton.topAnchor
        constraintEqualToAnchor:self.statusLabel.bottomAnchor
                       constant:20],
    [self.browseButton.centerXAnchor
    ...
    [self.logView.topAnchor
        constraintEqualToAnchor:self.browseButton.bottomAnchor
                       constant:20],
    ...
    [self.logView.trailingAnchor
        constraintEqualToAnchor:self.view.trailingAnchor
                       constant:-16],
    [self.logView.bottomAnchor
        constraintEqualToAnchor:self.view.safeAreaLayoutGuide.bottomAnchor
                       constant:-16]
  ]];

  [self log:@"HIAH Installer ready"];
  [self log:[NSString stringWithFormat:@"Install location: %@",
                                       [[HIAHFilesystem shared] appsPath]]];
}

- (void)browseTapped {
  UTType *ipaType = [UTType typeWithIdentifier:@"com.apple.itunes.ipa"];
  NSArray *types =
      ipaType ? @[ ipaType ] : @[ [UTType typeWithFilenameExtension:@"ipa"] ];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller
  ...
  [self log:[NSString
                stringWithFormat:@"Selected: %@", ipaURL.lastPathComponent]];
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [[NSFileManager defaultManager]
            copyItemAtURL:ipaURL
                    toURL:[NSURL fileURLWithPath:tempIPA]
                    error:&error];
        if (error) {
        ...
        // 3. Unzip .ipa
        NSString *unzipDir =
            [tempDir stringByAppendingPathComponent:@"extracted"];
        ...
        NSString *payloadDir =
            [unzipDir stringByAppendingPathComponent:@"Payload"];
        NSArray *contents =
        ...
        [[NSFileManager defaultManager] createDirectoryAtPath:appsFolder
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:nil];
        ...
        NSString *destPath =
            [appsFolder stringByAppendingPathComponent:appBundle];
        ...
        if (removeError && removeError.code != NSFileNoSuchFileError) {
          [self log:[NSString stringWithFormat:
                                  @"Warning: Could not remove existing app: %@",
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:execPath]) {
          [self
              log:[NSString stringWithFormat:@"ERROR: Executable not found: %@",
                                             exec]];
          [[NSFileManager defaultManager] removeItemAtPath:destPath error:nil];
          [[NSFileManager defaultManager] removeItemAtPath:tempDir error:nil];
          dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (permError) {
          [self log:[NSString stringWithFormat:
                                  @"Warning: Could not set permissions: %@",
                                  permError.localizedDescription]];
        } else {
        ...
        if (magic != MH_MAGIC_64 && magic != MH_CIGAM_64 &&
          ...
          [self log:[NSString
                        stringWithFormat:
                            @"ERROR: Not a valid Mach-O binary (magic: 0x%08x)",
                            magic]];
          [[NSFileManager defaultManager] removeItemAtPath:destPath error:nil];
          [[NSFileManager defaultManager] removeItemAtPath:tempDir error:nil];
          dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (patched) {
          [self log:[NSString
                        stringWithFormat:
                            @"✓ Successfully patched %@ to MH_BUNDLE", exec]];
        } else {
        ...
        [self log:@"Binary prepared for loading (signature will be handled by "
                  @"extension)"];
        ...
        [self log:[NSString stringWithFormat:@"Bundle ID: %@",
                                             bundleID ?: @"(none)"]];
        [self log:[NSString stringWithFormat:@"Bundle Name: %@",
                                             bundleName ?: @"(none)"]];
        ...
        [self
            log:[NSString stringWithFormat:@"✓ Installed: %@",
                                           [appBundle
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          self.statusLabel.text = [NSString
              stringWithFormat:@"✓ %@ installed successfully!",
...
- (BOOL)unzipFile:(NSString *)zipPath toDirectory:(NSString *)destPath {
  ...
  if (!(eocd[0] == 0x50 && eocd[1] == 0x4b && eocd[2] == 0x05 &&
        eocd[3] == 0x06)) {
  ...
  [self log:[NSString stringWithFormat:@"ZIP: %d entries, CD at 0x%x",
                                       numEntries, cdOffset]];
  ...
  for (int i = 0; i < numEntries; i++) {
    uint8_t header[46];
    if (fread(header, 1, 46, zipFile) != 46)
      break;

    // Verify central directory signature: 0x02014b50
    if (!(header[0] == 0x50 && header[1] == 0x4b && header[2] == 0x01 &&
          header[3] == 0x02))
      break;

    // Parse file header
    uint16_t compressionMethod = header[10] | (header[11] << 8);
    uint32_t compressedSize = header[20] | (header[21] << 8) |
                              (header[22] << 16) | (header[23] << 24);
    uint32_t uncompressedSize = header[24] | (header[25] << 8) |
    ...
    uint32_t localHeaderOffset = header[42] | (header[43] << 8) |
                                 (header[44] << 16) | (header[45] << 24);
    ...
    if (fileName[fileNameLen - 1] == '/') {
      ...
      if (compressionMethod == 0) {
        ...
        [[NSData dataWithBytesNoCopy:data
                              length:uncompressedSize] writeToFile:outPath
                                                        atomically:YES];
        ...
        NSData *compressed = [NSData dataWithBytesNoCopy:compressedData
                                                  length:compressedSize];
        NSData *decompressed = [compressed
            decompressedDataUsingAlgorithm:NSDataCompressionAlgorithmZlib
                                     error:nil];
        if (decompressed) {
          ...
          [[NSData dataWithBytesNoCopy:uncompressed
                                length:uncompressedSize] writeToFile:outPath
                                                          atomically:YES];
        }
      }
...
- (BOOL)application:(UIApplication *)app
  ...
  if ([url.pathExtension isEqualToString:@"ipa"]) {
    InstallerViewController *vc =
        (InstallerViewController *)[(UINavigationController *)self.window
...
int main(int argc, char *argv[]) {
  @autoreleasepool {
    return UIApplicationMain(argc, argv, nil,
                             NSStringFromClass([InstallerAppDelegate class]));
  }
}
