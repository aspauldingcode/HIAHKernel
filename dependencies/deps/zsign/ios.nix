{ pkgs, lib, fetchFromGitHub, xcode, openssl ? null }:
  ...
  ios-sim = pkgs.stdenv.mkDerivation rec {
    ...
    preConfigure = ''
      ...
      if [ -d "$OPENSSL_INCLUDE/openssl" ]; then
        ...
        echo "  ❌ OpenSSL not found at $OPENSSL_INCLUDE"
        echo "  Expected headers at: $OPENSSL_INCLUDE/openssl"
        exit 1
      fi
    '';
